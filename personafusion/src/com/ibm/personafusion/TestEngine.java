...
public class TestEngine {
	...
	void doit() {
		/*
		 * List<Person> people = new ArrayList<Person>(); String alphName =
		 * "abcdefghij"; for(int i=0; i<10; i++) { String name =
		 * alphName.charAt(i) + ""; List<Trait> traits = new ArrayList<Trait>();
		...
		// List<Person> genPeople =
		// PersonListGenerator.generateDistinctPeople(10);
		...
		/*
		 * Engine engine = new Engine(people); List<Person> peopleResults =
		 * engine.query(people.get(0));
		 * System.out.println(peopleResults.get(2).distToQueryPerson);
		 * System.out.println(peopleResults.toString());
		 * 
		 * Engine engine2 = new Engine(people); List<Person> peopleResultsName =
		 * engine2.query("c"); System.out.println(peopleResultsName.toString());
		 */
	}
