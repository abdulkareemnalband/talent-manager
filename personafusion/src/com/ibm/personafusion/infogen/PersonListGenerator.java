...
public class PersonListGenerator {
	...
	/**
	 * Will generate a new randomized list of N people with unique names and
	 * random backgrounds/skills. As of now, numPeople is currently maxxed at
	 * 100. Over 100 would give an error.
	 * 
	 * @param numPeople
	 *            - Number of people to generate overall (Currently maxed at
	 *            100)
	 * @param numCurrEmploy
	 *            - How many people are to be under the current company. Must be
	 *            less than numPeople
	 * @param currEmploy
	 *            - Name of the current company for the numCurrEmploy
	 * @return Empty List if numCurrEmploy > numPeople.
	 */
	public static List<Person> generateDistinctPeople(int numPeople,
			int numCurrEmploy, String currEmploy) {

		if (numCurrEmploy > numPeople)
		...
		try {
			URL url = new URL(
					"https://dl.dropboxusercontent.com/u/27101002/personafusion/imageLinks.txt");
			Scanner scan0 = new Scanner(url.openStream());

			// Set<String> nameList =
			// NameGenerator.generateDistinctFullNames(numPeople);
			Set<String> nameList = getNameList();
			for (String name : nameList) {
				// Call Watson to get traits. Call keyword converter to get
				// keywords.
				...
				String response = QuestionResponse
						.convertToFullString(qaResponses);
				...
				if (pplCounter <= (numCurrEmploy / 2)) {
					...
					List<String> techSkills = ResumeInfoGenerator
							.generateTechSkill("Dev");
					List<String> prevInfo = new ArrayList<String>();
					prevInfo.add(currEmploy);
					ResumeInfo ri = new ResumeInfo(techSkills, prevInfo);

					Person newPerson = new Person(name, traitList, imgURL, ri,
							Person.Role.DEV, new ArrayList<String>());

					/*
					 * ***TODO Restructure to read in a list of strings that
					 * represent the answers to interview questions
					 */
				...
				// Managers working at curr company
					...
					List<String> techSkills = ResumeInfoGenerator
							.generateTechSkill("Manager");
					List<String> prevInfo = new ArrayList<String>();
					prevInfo.add(currEmploy);
					ResumeInfo ri = new ResumeInfo(techSkills, prevInfo);

					Person newPerson = new Person(name, traitList, imgURL, ri,
							Person.Role.Manager, new ArrayList<String>());
				...
				// Dev not at curr company
					...
					List<String> techSkills = ResumeInfoGenerator
							.generateTechSkill("Dev");
					List<String> prevInfo = ResumeInfoGenerator
							.generatePrev("Dev");
					ResumeInfo ri = new ResumeInfo(techSkills, prevInfo);
