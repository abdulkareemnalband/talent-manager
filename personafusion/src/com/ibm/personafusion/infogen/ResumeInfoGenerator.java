...
public class ResumeInfoGenerator {
	...
	// final static String DevSkillURL =
	// "person_source/resumeinfo/dev_skill.txt";
	...
	// final static String ManSkillURL =
	// "person_source/resumeinfo/manager_skill.txt";
	final static int ManSkillLength = 74;

	// final static String ManPrevURL =
	// "person_source/resumeinfo/manager_prev.txt";
	...
	/**
	 * Takes in a String representing the role and returns a random amount of
	 * skills associated with the role. The number ranges from the min and max
	 * assigned in the class. If the role doesn't exist, returns null.
	 * 
	 * @param role
	 *            that the person is in. (For the purpose of POC, use only
	 *            either Dev or Manager).
	...
	public static List<String> generateTechSkill(String role) {
		...
		if (DevRole.equals(role)) {
			while (skillList.size() < skillNum) {
				...
				try {
					URL url = new URL(
							"https://dl.dropboxusercontent.com/u/27101002/personafusion/dev_skill.txt");
			...
			while (skillList.size() < skillNum) {
				...
				try {
					URL url = new URL(
							"https://dl.dropboxusercontent.com/u/27101002/personafusion/manager_skill.txt");
					scanner = new Scanner(url.openStream());
	...
	/**
	 * Takes in a String representing the role and returns a random amount of
	 * previous employers associated with the role. The number ranges from the
	 * min and max assigned in the class. If the role doesn't exist, returns
	 * null.
	 * 
	 * @param role
	 *            that the person is in. (For the purpose of POC, use only
	...
	public static List<String> generatePrev(String role) {
		...
		if (DevRole.equals(role)) {
			while (prevList.size() < prevNum) {
				...
				try {
					URL url = new URL(
							"https://dl.dropboxusercontent.com/u/27101002/personafusion/dev_prev.txt");
					scanner = new Scanner(url.openStream());
			...
			while (prevList.size() < prevNum) {
				...
				try {
					URL url = new URL(
							"https://dl.dropboxusercontent.com/u/27101002/personafusion/manager_prev.txt");
					scanner = new Scanner(url.openStream());
