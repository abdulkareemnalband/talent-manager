...
/**
 * A wrapper for accessing the Watson User Model API. Usage: WatsonUserModeller
 * WUM = new WatsonUserModeller(); List<Trait> traits =
...
public class WatsonUserModeller {
	...
	public List<Trait> getTraitsList(String text) {
		...
		for (Map<String, String> m : lm) {
			...
			if (id != null) {
				double pct = 0.0;
				/**
				 * NOTE: val == null means that this id is the name of a trait
				 * 'category'; e.g. values, personality. We do not add
				 * categories to the returned list of traits.
				 */
				if (val != null) {
	...
	/**
	 * Produce a visualization of a Person's traits based on the text of their
	 * tweets. Uses the Watson API.
	 **/
	...
	/**
	 * Get a personality profile from Watson based on the input text. Returns a
	 * string of JSON.
	 **/
	...
	private static void formatTree(JSONObject node, int level,
